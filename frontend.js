...
jQuery(this).prepend('<div class="url_preview_loading"></div>')
});*/
...
jQuery('#aw-whats-new-submit').click(function(){
				...
				if(!jQuery('#whats-new-content').find('.url_preview_loading').get(0))
					jQuery('#whats-new-content').append("<div class='url_preview_loading'></div>");
				
				jQuery(document).ajaxComplete(function(e, xhr, options){
					submit_butt.show();
					submit_butt.removeAttr('disabled');
					jQuery('#whats-new-content').find('.url_preview_loading').remove();
